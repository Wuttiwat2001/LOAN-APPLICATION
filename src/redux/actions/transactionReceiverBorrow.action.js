...
export const setTransactionFetchingToState = () => ({
  type: TRANSACTION_RECEIVER_BORROW_FETCHING,
});

export const setTransactionSuccessToState = (payload) => ({
  type: TRANSACTION_RECEIVER_BORROW_SUCCESS,
  payload,
});

export const setTransactionFailedToState = (payload) => ({
  type: TRANSACTION_RECEIVER_BORROW_FAILED,
  payload,
});

export const setTransactionClearToState = () => ({ type: TRANSACTION_RECEIVER_BORROW_CLEAR });
  ...
  return async (dispatch) => {
    try {
      ...
      if (loginReducer.isLogin) {
        ...
        const response = await api.post(`${server.TRANSACTION_URL}/receiver?page=${page}&pageSize${pageSize}`,{
          userId: userId,
          search: search,
